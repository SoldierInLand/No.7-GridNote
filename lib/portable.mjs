...
const renderRichValue = (value) =>
  /<\/?[a-z][\s\S]*>/i.test(String(value))
    ? sanitizeRichText(String(value))
    : escapeHtml(value);
      ...
      if (candidate.type === "rich-text") {
        ...
        blocks.push({
          ...
          color: /^#[0-9a-f]{6}$/i.test(candidate.color)
            ...
            : "#dce6d5",
          text: (() => {
            const value = safeText(candidate.text);
            return /<\/?[a-z][\s\S]*>/i.test(value)
              ? sanitizeRichText(value)
              : value;
          })(),
...
function renderBlock(block, assets) {
  ...
  if (block.type === "table") {
    const rows = block.cells
      .map(
        (row, rowIndex) =>
          `<tr>${row.map((cell) => `<${rowIndex === 0 ? "th" : "td"}>${renderRichValue(cell)}</${rowIndex === 0 ? "th" : "td"}>`).join("")}</tr>`,
      )
      .join("");
  ...
  if (block.type === "shape") {
    const color = /^#[0-9a-f]{6}$/i.test(block.color)
      ? block.color
      : "#dce6d5";
    return `<div class="block shape-block shape-${escapeHtml(block.shape)}" data-block-id="${escapeHtml(block.id)}" data-block-type="shape" data-column="${block.column}" data-row="${block.row}" data-column-span="${block.columnSpan}" data-row-span="${block.rowSpan}" style="--column:${block.column};--row:${block.row};--column-span:${block.columnSpan};--row-span:${block.rowSpan};background:${color};color:${contrastTextColor(color)}">${renderRichValue(block.text)}</div>`;
  }
