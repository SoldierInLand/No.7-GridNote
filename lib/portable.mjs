...
function renderBlock(block, assets) {
  ...
  if (block.type === "shape") {
    const color = /^#[0-9a-f]{6}$/i.test(block.color)
      ? block.color
      : "#dce6d5";
    return `<div class="block shape-block shape-${escapeHtml(block.shape)}" data-block-id="${escapeHtml(block.id)}" data-block-type="shape" data-column="${block.column}" data-row="${block.row}" data-column-span="${block.columnSpan}" data-row-span="${block.rowSpan}" style="--column:${block.column};--row:${block.row};--column-span:${block.columnSpan};--row-span:${block.rowSpan};background:${color}">${renderRichValue(block.text)}</div>`;
  }
