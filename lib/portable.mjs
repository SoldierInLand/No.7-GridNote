...
export function createPortableFiles(notebook) {
  const encoded = encodeURIComponent(JSON.stringify(notebook));
  ...
  const pages = notebook.pages
    .map(
      (page) =>
        `<section class="page" data-page-id="${escapeHtml(page.id)}"><h1 class="page-title">${escapeHtml(page.title)}</h1>${page.blocks.map(renderBlock).join("")}</section>`,
    )
    .join("");
